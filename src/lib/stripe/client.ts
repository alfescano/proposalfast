...
/**
 * Read env at runtime via a computed key.
 * Next.js statically replaces `process.env.NAME` at build; Vercel Sensitive
 * secrets are unavailable then, so the server bundle would bake in `undefined`.
 */
