...
2. **`AUTH_SECRET`**, **`AUTH_URL`**, **`NEXT_PUBLIC_APP_URL`** — `https://proposefast.com`.
...
5. **Stripe** — `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, publishable key, Pro/Business price IDs. Webhook URL: `https://proposefast.com/api/webhooks/stripe`.
6. **S3/R2** — logos, PDFs, drawn signatures if stored.
7. **DNS** — Namecheap Advanced DNS for `proposefast.com` (A `@` → `76.76.21.21`, CNAME `www` → `cname.vercel-dns.com`, or the records Vercel shows); HTTPS issued.
