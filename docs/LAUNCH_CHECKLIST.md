...
2. **`AUTH_SECRET`**, **`AUTH_URL`**, **`NEXT_PUBLIC_APP_URL`** — `https://proposalfast.com`.
...
5. **Stripe** — `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, publishable key, Pro/Business price IDs. Webhook URL: `https://proposalfast.com/api/webhooks/stripe`.
6. **S3/R2** — logos, PDFs, drawn signatures if stored.
7. **DNS** — apex + optional `www` at Vercel; HTTPS issued.
